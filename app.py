...
try:
    ...
    PIL_AVAILABLE = False
# https://just-insight-production-tds-p2.up.railway.app/api
# https://github.com/Anshika310/TDS_project2_1
...
if not GEMINI_KEYS:
    raise RuntimeError(
        "No Gemini API keys found. Please set them in your environment.")
...
def run_agent_safely(llm_input: str) -> Dict:
    ...
    try:
        ...
        code = parsed["code"]
        # Log agent-generated code and Executoin Output Inside
        # Doing this in both run_agent_safely and run_agent_safely unified.
        logger.info("Agent-generated code:\n%s", code)
        ...
        # Log agent code and execution output
        logger.info("Execution result:\n%s", json.dumps(exec_result, indent=2))
...
# Change 1: Added falllbacks and transparent_png_base64 img(vars setup).
FALLBACKS = {
    float: 0.0,
    int: 0,
    str: "",
...
# Transparent PNG fallback for image fields
TRANSPARENT_PNG_BASE64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO3n+9QAAAAASUVORK5CYII="
...
async def analyze_data(request: Request):
    try:
        ...
        '''
        # Post-process key mapping & type casting _ version 1:
        if keys_list and type_map:
            ...
            result = mapped
            '''
        # Post-process key mapping & type casting _ version 2:
        if keys_list and type_map:
            mapped = {}
            for idx, q in enumerate(result.keys()):
                key = keys_list[idx] if idx < len(keys_list) else f"q{idx}"
                caster = type_map.get(key, str)
                try:
                    val = result[q]
                    if isinstance(val, str) and val.startswith("data:image/"):
                    ...
                    if val in (None, ""):
                        # Use fallback
                        fallback = TRANSPARENT_PNG_BASE64 if caster == str and "chart" in key.lower(
                        ) else FALLBACKS.get(caster, "")
                        mapped[key] = fallback
                        logger.warning(
                            "Missing value for key '%s', using fallback", key)
                    else:
                    ...
                    mapped[key] = fallback
                    logger.warning(
                        "Casting failed for key '%s': %s. Using fallback.", key, str(e))
            result = mapped
...
def run_agent_safely_unified(llm_input: str, pickle_path: str = None) -> Dict:
    ...
    try:
        ...
        results_dict = exec_result.get("result", {})
        # Change 3: added 3 lines of missing keys log.(Log missing answers.)
        missing_keys = [q for q in questions if q not in results_dict]
        if missing_keys:
            logger.warning(
                "Agent did not return answers for: %s", missing_keys)
...
# seconds per key/model simple ping test (sync tests run in threadpool)
DIAG_LLM_KEY_TIMEOUT = 30
DIAG_PARALLELISM = 6       # how many thread workers for sync checks
# Playwright/duckdb tests run only if true (they can be slow)
...
def _env_check(required=None):
    ...
    out = {}
    for k in required:
        out[k] = {"present": bool(os.getenv(k)), "masked": (
            os.getenv(k)[:4] + "..." + os.getenv(k)[-4:]) if os.getenv(k) else None}
...
def _system_info():
    ...
    try:
        info["tmp_free_gb"] = round(shutil.disk_usage(
            tempfile.gettempdir()).free / 1024**3, 2)
    except Exception:
...
async def check_llm_keys_models():
    ...
    for model in (_MODEL_HIERARCHY or ["gemini-2.5-pro"]):
        ...
        for key in _GEMINI_KEYS:
            tasks.append(run_in_thread(_test_gemini_key_model,
                         key, model, timeout=DIAG_LLM_KEY_TIMEOUT))
        ...
        for key, res in zip(_GEMINI_KEYS, completed):
            if isinstance(res, Exception):
                model_summary["attempts"].append({"key_mask": (
                    key[:4] + "..." + key[-4:]) if key else None, "ok": False, "error": str(res)})
            else:
                # res is dict returned by _test_gemini_key_model
                model_summary["attempts"].append(
                    {"key_mask": (key[:4] + "..." + key[-4:]) if key else None, **res})
                if res.get("ok"):
                    any_ok = True
...
async def diagnose(full: bool = Query(False, description="If true, run extended checks (duckdb/playwright)")):
    ...
    for name, coro in tasks.items():
        try:
            ...
            results[name] = {"status": "error", "error": str(
